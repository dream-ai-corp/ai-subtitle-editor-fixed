describe("Final Purchase Workflow", () => {
  it("should complete the full purchase workflow with correct Stripe integration", () => {
  ...
  it("should test Stripe API integration with correct product IDs", () => {
  ...
  it("should test all three plans with correct product IDs", () => {
    // Test Basic plan
    cy.visit("/purchase");
    cy.wait(3000);
    cy.contains("Basic").parent().parent().parent().click({ force: true });
    ...
    cy.intercept("POST", "/api/subscriptions/create-checkout-session/", {
      statusCode: 200,
      body: {
        session_id: "cs_test_basic",
        url: "https://checkout.stripe.com/pay/cs_test_basic"
      }
    }).as("createCheckoutSessionBasic");

    cy.contains("Proceed to Payment").click({ force: true });
    cy.wait("@createCheckoutSessionBasic").then((interception) => {
      expect(interception.request.body.plan_id).to.equal("prod_SoMRaG2MTtodR1");
      cy.log("✅ Basic plan product ID correct");
    });
    ...
    }).as("createCheckoutSessionPro");

    cy.contains("Proceed to Payment").click({ force: true });
    cy.wait("@createCheckoutSessionPro").then((interception) => {
      expect(interception.request.body.plan_id).to.equal("prod_SoMSs2YZE1zCEq");
      cy.log("✅ Professional plan product ID correct");
    });

    // Test Enterprise plan
    cy.visit("/purchase");
    cy.wait(3000);
    cy.contains("Enterprise").parent().parent().parent().click({ force: true });
    cy.wait(1000);
    cy.scrollTo("bottom");
    cy.wait(1000);
    ...
    cy.intercept("POST", "/api/subscriptions/create-checkout-session/", {
      statusCode: 200,
      body: {
        session_id: "cs_test_enterprise",
        url: "https://checkout.stripe.com/pay/cs_test_enterprise"
      }
    }).as("createCheckoutSessionEnterprise");

    cy.contains("Proceed to Payment").click({ force: true });
    cy.wait("@createCheckoutSessionEnterprise").then((interception) => {
      expect(interception.request.body.plan_id).to.equal("prod_SoMS0GmORJMfdo");
      cy.log("✅ Enterprise plan product ID correct");
    });

    cy.log("✅ All three plans tested with correct Stripe product IDs");
  });
});
