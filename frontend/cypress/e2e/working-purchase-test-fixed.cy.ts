describe("Working Purchase Test - Fixed", () => {
  it("should complete the full purchase workflow with proper plan selection", () => {
    ...
    // Step 7: Click on the plan card to select it (using the correct selector)
    cy.get(".plan-card").first().click({ force: true });
    cy.log("✅ Selected Basic plan by clicking on plan card");
    ...
    cy.get(".plan-card").eq(1).click({ force: true }); // Second plan card (Professional)
    cy.log("✅ Selected Professional plan by clicking on plan card");
  ...
  it("should test plan selection and proceed button visibility", () => {
    ...
    // Check if plan cards exist
    cy.get(".plan-card").should("have.length", 3);
    cy.log("✅ Found 3 plan cards");
